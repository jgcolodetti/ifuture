...
export default function RestaurantCard() {
    const { restaurants, filteredRestaurants, filteredCategory, categoryFiltered } = useContext(GlobalContext)
    return (
        <div>
            {categoryFiltered ? filteredCategory.map((item) => {
                return <Flex justify={'center'} flexDir={'column'} align={'center'} marginY={'0.5rem'}>
